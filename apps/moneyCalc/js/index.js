const generateId = () => `matrix${Math.round(Math.random() * 1e8).toString(16)}`;
...
const addOperation = event => {
  ...
  if(operationNameValue !== '' && operationAmountValue !== '') {
    ...
    dbOperation.push(operationData);
    init();
  } else {
...
const deleteOperation = event => {
  ...
  if(target.classList.contains('history__delete')) {
    const indexEl = dbOperation.findIndex(el => el.id === target.dataset.id);
    dbOperation.splice(indexEl, 1);
    init();
  }
};
